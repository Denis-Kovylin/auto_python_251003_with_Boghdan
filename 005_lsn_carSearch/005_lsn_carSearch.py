...
for brand, specs in car_data.items():
    ...
    year: int = specs[1]
    engine: float = specs[2]
    price: int = specs[4]

    if year >= search_criteria[0] and engine >= search_criteria[1] and price <= search_criteria[2]:
      search_result[brand] = specs

sorted_result_list = list(search_result.items())
sorted_result_list.sort(key=lambda elem: elem[1][4])

print(f"За критериями підбору знайдено: {len(sorted_result_list)} автівок")
print(f"П'ять найдешевших: ")

for car in range(min(5, len(sorted_result_list))):
  brand, specs = sorted_result_list[car]
  print(brand, specs)
